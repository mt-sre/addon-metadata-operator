...
func TestExtractorFileBasedAndSQLCatalogs(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.indexImage, func(t *testing.T) {
			...
			requireContainsAll(t, bundleImages, tc.expectedBundleImages)

			cachedBundleImages := cache.GetBundleImages(tc.indexImage, tc.pkgName)
			requireContainsAll(t, cachedBundleImages, tc.expectedBundleImages)
		})
	}
}
...
func TestIndexExtractorListAllBundles(t *testing.T) {
	...
	require.NoError(t, err)
	requireContainsAll(t, bundleImages, expectedBundleImages)

	cachedBundleImages := cache.GetBundleImages(indexImage, allBundlesKey)
	requireContainsAll(t, cachedBundleImages, expectedBundleImages)
...
func requireContainsAll(t *testing.T, images []string, expectedImages []string) {
	require.Equal(t, len(images), len(expectedImages))
	for _, img := range images {
		require.Contains(t, expectedImages, img)
	}
}
